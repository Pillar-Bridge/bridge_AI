...
class get_recommendation:
    ...
    def inverted_lang(self, language):
        if language == 'ko':
            return '한국어'
        else:
    ...
    def generate_responses(self, place, question, lang):
        ...
        print(response.text)
        self.save_response_as_json(place, question, response.text, lang) # JSON 파일로 응답 저장
        ...
        #         "role":"user",
        #         "parts": [
        #             f"특정장소({place})에서 상대방이 특정 질문({question})라는 질문에 대해 어떤 상황인지 명사로 (key : situation)설명하고, 이 상황에서 상대방의 질문에 대응할 수 있는 답변(key : response)을 5개 생성해주세요. 더불어 답변에 대한 비슷한 종류 문장(key : similar)과 반의어 문장(key : antonym), 고유 명사들 대신 사용할 수 있는 후보 명사들(key : nouns, value : [후보 명사 리스트])도 각 답변 문장마다 생성해주세요. 최종적으로 만든 결과물을 {lang}로 만들어주시고,json 형식의 파일로 저장해주세요."
        #         ]
        #     }
        # ]
        ...
        #     # JSON 파일로 응답 저장
        #     self.save_response_as_json(place, question, response.text, lang)

    def check_json(self, place, question, lang, json_str):
        input_str = json_str
        ...
        if "```json" in input_str:
            ...
            self.generate_responses(place, question, lang)
            return input_str, False
        
    def save_response_as_json(self, place, question, response, lang):
        # 현재 시간을 사용하여 파일명 생성
        # nowadays = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
        
        response, json_tag = self.check_json(place, question, lang, response)
        ...
        if json_tag:
            ...
            self.generate_responses(place, question, lang) #원하는 형식이 아니면 다시 실행  
    ...
    def json_input(self):
        ...
        with open('/home/bmegpu02/dh/gdsc/recomm/input/testInput.json', 'r', encoding='utf-8') as f:
            data = json.load(f)
        return data['place'], data['text'], self.inverted_lang(data['lang'])
    ...
    def run(self):
        ...
        place, question, lang = self.json_input()
        self.generate_responses(place, question, lang) #<-- 실행부분
        # end_time = time.time()
